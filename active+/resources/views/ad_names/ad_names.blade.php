<x-app-layout>
   <section id="tabs-with-icons">
      <div class="row match-height">
               ...
               <div class="card-content">
                  <div class="card-body">
                     <div class="table-responsive">
                        <table class="table table-striped table-bordered ad_name" style="width:100%;">
                           <thead>
                              <tr>
                                 <th>Ad Name Id</th>
                                 <th>&nbsp;&nbsp;&nbsp;Action&nbsp;&nbsp;&nbsp;</th>
               ...
               <div class="modal-content">
                  <section class="contact-form">
                     <div class="modal-header bg-primary white">
                        <h5 class="modal-title white">Ad Name</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                           <span aria-hidden="true">&times;</span>
                        </button>
                     </div>
                     <form method="post" action="{{ route('ad_names_submit') }}">
                        @csrf
                              ...
                              <div class="col-lg-12">
                                 <div class="form-group">
                                    <fieldset class="form-group floating-label-form-group"><b>Ad Name <sup class="text-danger" style="font-size: 13px;">*</sup> :</b>
                                       <input type="text" id="" required name="ad_name" class="name form-control" placeholder="Ad Name">
                                    </fieldset>
                                 </div>
                              </div>
                           </div> 
         ...
         <div class="modal fade" id="edit_modal_ad_names"  role="dialog" aria-labelledby="edit_modal_ad_names" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered  modal-md" role="document">
               <div class="modal-content">
                  <section class="contact-form">
                     <div class="modal-header bg-primary white">
         ...
         @php
         $DeleteTableName="ad_names";
         $DeleteColumnName="ad_name_id";
         @endphp
         @include('delete')
         <x-slot name="page_level_scripts">
            <script type="text/javascript">
               $(function () {
                  var table = $('.ad_name').DataTable({
                     processing: true,
                     serverSide: true,
                     order: [[ 0, "desc" ]],
                     ajax: "{{ route('ad_names') }}",
                     columns: [
                     {data: 'ad_name_id', name: 'a.ad_name_id'},
                     {data: 'action', name: 'a.action', orderable: false, searchable: false},
                     {data: 'ad_name', name: 'a.ad_name'},
               ...
               $(document).on('click', '.edit_model_btn', function(){
                  var ad_name_id=$(this).closest("tr").find("td:eq(0)").text();
                  $.ajax({
                     url: 'ad_names_edit/'+ad_name_id,
                     type: "GET",
                     data : {"_token":"{{ csrf_token() }}"},
                     success:function(data) {
                        $("#menu_group_edit_modal_form").html(data);
                        $("#edit_modal_ad_names").modal('show');
                     }
                  });
               });
