...
class ProductCategoriesController extends Controller
{
    public function __construct()
   {
    ...
    public function product_categories(Request $request){
        if ($request->ajax()) {
            $data = DB::table('product_categories as a')->where('a.deleted','No')->select(['a.product_category_id',
        ...
        $GetCustomFields=DB::table('custom_fields')->where('field_page', 21)->where('deleted', 'No')->get();
        $GetCustomFieldTypes=DB::table('custom_fieldtype')->where('deleted', 'No')->get();
        return view('product_categories.product_categories',compact('GetCustomFields','GetCustomFieldTypes'));
    }

    public function product_categories_submit(Request $request){
        $createdby = Auth::id();
        $updatedby = Auth::id();
        if(isset($request->product_category_id))
        {
            ...
            $AttachmentsFolder="public/uploads/ProductCategories/";
           ...
           foreach($_FILES as $key => $value) 
           {
            ...
            $data = array('product_category_name' => $request->product_category_name, 'custom_fields' => $CustomFieldJsonValue, 'updated_by' => $updatedby, 'updated_at' => Now());
            $product_categorie_update=DB::table('product_categories')->where('product_category_id',$request->product_category_id)->update($data);
        }
        else
        {
            $CustomFieldValuesArr=array();
         ...
         $CustomFieldJsonValue=json_encode($CustomFieldValuesArr);
            $data = array('product_category_id' => $request->product_category_id, 'product_category_name' => $request->product_category_name, 'custom_fields' => $CustomFieldJsonValue, 'created_by' => $createdby, 'created_at' => Now());

            $product_categorie_add=DB::table('product_categories')->insert($data);
        }
        return redirect('product_categories');
    }

    public function product_categories_edit(Request $request){
        $product_categories_details=DB::table('product_categories')->where('product_category_id', $request->product_category_id)->first();
        $GetCustomFields=DB::table('custom_fields')->where('field_page', 21)->where('deleted', 'No')->get();
        $GetCustomFieldTypes=DB::table('custom_fieldtype')->where('deleted', 'No')->get();
        $data='<div class="modal-body">
        <input type="hidden" name="product_category_id" value="'.$product_categories_details->product_category_id.'">
        <div class="row">
        <div class="col-lg-12">
        ...
        <input type="text" id="" required name="product_category_name" class="name form-control" placeholder="Menu Name" value="'.$product_categories_details->product_category_name.'">
        </fieldset>
        </div>
        </div>
        </div>
                ...
                <div class="form-group">
                    <fieldset class="form-group floating-label-form-group"><label>'.$GetCustomField->field_name.' ';if($GetCustomField->required_field=="Yes"){$data.='<sup style="color:red;font-size:12px;">*</sup>';}$data.=' :</label><br>';
                       ...
                       if($GetCustomField->field_type==1 OR $GetCustomField->field_type==2 OR $GetCustomField->field_type==4 OR $GetCustomField->field_type==5 OR $GetCustomField->field_type==8 OR $GetCustomField->field_type==10){
                          $product_category_value=$product_categories_details->custom_fields;
