<x-app-layout>
   <section class="tabs-with-icons">
      <div class="row">
         <div class="col-xl-10 col-md-8 col-12 printable_content" id="print_quotation">
            <div class="card">
               <div class="card-body p-2">
                  <div class="card-header px-0">
                     <div class="row">
                        <div class="col-md-12 col-lg-7 col-xl-4 mb-50">
                           <!-- <span class="invoice-id font-weight-bold">Quotation# </span> -->
                           <img src="{{ asset('public/accsource/mainlogo.png') }}" alt="company-logo" height="80" width="80" style="margin-left:5px ;">
                        </div>
                  ...
                  <div class="col-6"  align="right">
                     ...
                     <div class="company-address mb-1">
                        <span align="center">Quotation #:  {{ $quotation_view->QuotationId }}</span>
                     </div>
                     <div class="company-address mb-1">
                        <span align="center">Quote Date :  {{ $quotations->date_issue }}</span>
                     </div>
                  </div>
               </div>
               <hr>
                                 ...
                                 @if($GetCustomFieldType->field_type_id==$GetCustomField->field_type)
                                    @if($CustomFieldsValueArr!='')
                                       @if(array_key_exists($custom_field_id,$CustomFieldsValueArr)) 
                                          <p><?= $CustomFieldsValueArr[$custom_field_id] ?><a href="public/uploads/quotations/<?= $CustomFieldsValueArr[$custom_field_id] ?>">Click to View</a></p>
                                       @else
                        ...
                        <tr>
                           ...
                           @if($quotation_item->Product!='')
                              @foreach($products as $product)
                                 ...
                                 <span style="color:gray; font-style: italic; font-size: 13px;">
                                    {{ $quotation_item->ProductDescription }}</span>
                                 </td>
                                 @endif
                              @endforeach
                           @endif
                           @if($quotation_item->Service!='')
                              @foreach($services as $service)
                                 ...
                                 <span style="color:gray; font-style: italic; font-size: 13px;">
                                    {{ $quotation_item->ServiceDescription }}</span>
                                 </td>
                                 @endif
                              @endforeach
                           @endif
                           <td>{{ $quotation_item->Cost }}</td>
                           <td>{{ $quotation_item->Quantity }}</td>
                           <td class="font-weight-bold"><i class="fa fa-inr text-light"></i>{{ $quotation_item->Amount }}</td>
                        </tr>
                        @php $i++; @endphp
                        @endforeach
                     </tbody>
                     ...
                     <div class="col-8 col-sm-6 d-flex justify-content-end mt-75">
                        <ul class="list-group cost-list">
                           <li class="list-group-item each-cost border-0 p-50 d-flex justify-content-between">
                              ...
                              <span class="cost-value"><i class="fa fa-inr text-light"></i> {{ $quotations->total_amount }}.00</span>
                           </li>
                           <div class="<?php if($gsttype==1){echo"hidden";} ?>">
                              <li class="list-group-item each-cost border-0 p-50 d-flex justify-content-between">
                                 <span class="cost-title mr-2">CGST&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;-</span>
                                 <span class="cost-value"><i class="fa fa-inr text-light"></i> {{ $quotations->total_cgst }}.00</span>
                              </li>
                           </div>
                           <div class="<?php if($gsttype==1){echo"hidden";} ?>">
                              <li class="list-group-item each-cost border-0 p-50 d-flex justify-content-between">
                                 <span class="cost-title mr-2">SGST&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;-</span>
                                 <span class="cost-value"><i class="fa fa-inr text-light"></i> {{ $quotations->total_sgst }}.00</span>
                              </li>
                           </div>
                           <div class="<?php if($gsttype!=1){echo"hidden";} ?>">
                              <li class="list-group-item each-cost border-0 p-50 d-flex justify-content-between">
                                 <span class="cost-title mr-2">IGST&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;-</span>
                                 <span class="cost-value"><i class="fa fa-inr text-light"></i> {{ $quotations->total_igst }}.00</span>
                              </li>
                           </div>
                           <li class="dropdown-divider"></li>
                           <li class="list-group-item each-cost border-0 p-50 d-flex justify-content-between">
                              <span class="cost-title mr-2">Quotation Total -</span>
                              <span class="cost-value"><i class="fa fa-inr text-light"></i> {{ $quotations->grand_total }}.00</span>
                           </li>
                           <li class="dropdown-divider"></li>
                        </ul>
                     </div>
      ...
      <div class="col-xl-2 col-md-4 col-12 action-btns">
         <div class="card">
            <div class="card-body p-2">
               <!-- <a href="#" class="btn btn-primary btn-block mb-1"> <i class="feather icon-check mr-25 common-size"></i> Send Quotation</a> -->
               <a class="btn btn-warning btn-block btn-sm mb-1 print-invoice" onclick="codespeedy()"><i class="feather icon-printer mr-25 common-size"> Print</i></a>
               <a href="{{ route('quotations_edit',base64_encode($quotation_view->QuotationId)) }}" class="btn btn-sm btn-primary btn-block mb-1"><i class="fa fa-edit mr-25 common-size"> Edit</i> </a>
               <a target="_blank" href="{{ route('quotations_pdf', base64_encode($quotation_view->QuotationId)) }}" class="btn btn-sm btn-info btn-block mb-1"><i class="fa fa-download mr-25 common-size"> PDF</i> </a>
               <a href="{{ route('quotation_send_mail',base64_encode($quotation_view->QuotationId)) }}" class="btn btn-sm btn-success btn-block mb-1"><i class="fa fa-paper-plane  mr-25 common-size"> Mail</i> </a>
            </div>
         </div>
      </div>
   </section>
   ...
   <x-slot name="page_level_scripts">
      <script type="text/javascript">
         function codespeedy(){
            var print_div = document.getElementById("print_quotation");
