<x-app-layout>
         ...
         <div class="col-xl-12 col-lg-12">
            <div class="card">
               <div class="card-header">
                  <h4 class="card-title">Leave Types
                  ...
                  <div class="heading-elements">
                     <a>
                        <button  type="submit" class="btn btn-primary" data-toggle="modal" data-target="#AddModal">
                           <i class="fa fa-plus"></i> Leave Type
                        </button>
                     </a>
                  </div>
               </div>
               <div class="card-content">
                  <div class="card-body">
                     <div class="table-responsive">
                        <table class="table table-striped table-bordered leave_type" style="width:100%;">
                           <thead>
                              <tr>
                                 <th>Leave Type Id</th>
                                 <th>&nbsp;&nbsp;&nbsp;&nbsp;Action&nbsp;&nbsp;&nbsp;&nbsp;</th>
         ...
         <div class="modal-content">
            <section class="contact-form">
               <div class="modal-header bg-primary white">
                  <h5 class="modal-title white">Leave Type</h5>
                  <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                     <span aria-hidden="true">&times;</span>
                  </button>
               </div>
               <form method="post" action="{{ route('leave_types_submit') }}">
                  @csrf
                        ...
                        <div class="col-lg-12">
                           <div class="form-group">
                              <fieldset class="form-group floating-label-form-group"><b>Leave Type Name <sup class="text-danger" style="font-size: 13px;">*</sup> :</b>
                                 <input type="text" id="" required name="leave_type_name" class="name form-control" placeholder="Leave Type Name">
                              </fieldset>
                           </div>
                        </div>
                     </div> 
   ...
   @php
   $DeleteTableName="leave_types";
   $DeleteColumnName="leave_type_id";
   @endphp
   @include('delete')
   <x-slot name="page_level_scripts">
      <script type="text/javascript">
         $(function () {
            var table = $('.leave_type').DataTable({
               processing: true,
               order: [[ 0, "desc" ]],
               serverSide: true,
               ajax: "{{ route('leave_types') }}",
               columns: [
               {data: 'leave_type_id', name: 'a.leave_type_id'},
               {data: 'action', name: 'a.action', orderable: false, searchable: false},
               {data: 'leave_type_name', name: 'a.leave_type_name'},
         ...
         $(document).on('click', '.edit_model_btn', function(){
            var leave_type_id=$(this).closest("tr").find("td:eq(0)").text();
            $.ajax({
               url: 'leave_types_edit/'+leave_type_id,
               type: "GET",
