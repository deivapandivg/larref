         ...
         <div class="col-xl-12 col-lg-12 col-12">
            <div class="card">
               <div class="card-header">
                  <h4 class="card-title">Proforma Invoice Edit
                  ...
                  <form method="post" action="{{ route('proforma_invoices_submit') }}" id="form" enctype="multipart/form-data">
                     ...
                     <div class="row ml-2 mr-2">
                        <div class="col-xl-4">
                           <div class="form-group">
                              ...
                              <input type="hidden" name="proforma_invoice_id" value="{{ $proforma_invoice->proforma_invoice_id }}">
                              ...
                              <select class="form-control border-primary select2 form-select" name="client_id" data-placeholder="Choose one" style="width:100%;">
                                 ...
                                 <option value="{{ $client->client_id }}"{{ ($proforma_invoice->client_id==$client->client_id) ? 'selected' : '' }}>{{ $client->client_name }}</option>
                                 @endforeach
                              </select>
                           </div>
                        </div>
                        ...
                        <div class="col-xl-4 DateFilters">
                           <div class="form-group">
                              ...
                              <input type="date" name="qualification_date"  value="{{ $proforma_invoice->date_issue }}"  class="form-control" id="qualification_date">
                           </div>
                        </div>
                        <div class="col-xl-4 DateFilters">
                           <div class="form-group">
                              <label class="label-control">Valid Date</label>
                              <input type="date" name="valid_date" id="valid_date" value="{{ $proforma_invoice->date_due }}" class="form-control">
                           </div>
                        </div>
                     </div>
                     <div class="row ml-2 mr-2">
                     @if($proforma_invoice_details->custom_fields!='')
                           ...
                           <div class="form-group">
                              <fieldset class="form-group floating-label-form-group"><label>{{ $GetCustomField->field_name }} @if($GetCustomField->required_field=="Yes")<sup style="color:red;font-size:12px;">*</sup>';@endif :</label><br>
                                 ...
                                 @if($GetCustomField->field_type==1 OR $GetCustomField->field_type==2 OR $GetCustomField->field_type==4 OR $GetCustomField->field_type==5 OR $GetCustomField->field_type==8 OR $GetCustomField->field_type==10)
                                    @php
                                       $proforma_invoice_value=$proforma_invoice_details->custom_fields;
                                       $CustomFieldsValueArr=json_decode($proforma_invoice_value, true);
                                 ...
                                 @elseif($GetCustomField->field_type==6)
                                    ...
                                    @foreach($GetCustomFieldTypes as $GetCustomFieldType)
                                       @if($GetCustomFieldType->field_type_id==$GetCustomField->field_type)
                                          @if($CustomFieldsValueArr!='')
                                             @if(array_key_exists($custom_field_id,$CustomFieldsValueArr))
                                                <?=$CustomFieldsValueArr[$custom_field_id]?>&nbsp;&nbsp;<a href="public/uploads/proforma_invoices/<?=$CustomFieldsValueArr[$custom_field_id]?>" target="_blank"><i class="fa fa-eye"></i></a><input type="<?=$GetCustomFieldType->field_type?>" class="name form-control" value="<?=$CustomFieldsValueArr[$custom_field_id]?>" name="Custom-<?=$GetCustomField->custom_field_id?>">
                                             @else
                                                <input type="<?=$GetCustomFieldType->field_type?>" class="name form-control" value="" placeholder="<?=$GetCustomField->field_name?>" name="Custom-<?=$GetCustomField->custom_field_id?>">
                                             @endif
                                          @else
                     ...
                     <div class="row">
                        <div class="col-xl-4 col-lg-4">
                           <input type="radio" name="GSTType" id="IGST" class="GSTTypeIn" value="1"{{ ($proforma_invoice->igst_type==1) ? 'checked' : '' }}>&nbsp;&nbsp;<label class="label-control" for="IGST">IGST</label>&nbsp;&nbsp;
                           <input type="radio" name="GSTType" id="CGSTType" class="GSTTypeIn" value="2"{{ ($proforma_invoice->igst_type!=1) ? 'checked' : '' }}>&nbsp;&nbsp;<label class="label-control" for="CGSTType">CGST & SGST</label>
                        </div>
                        <div class="col-xl-4 col-lg-4">
                        </div>
                        <div class="col-xl-4 col-lg-4">
                     ...
                     <table class="table-striped table-responsive" border="0" id="mytable">
                        ...
                        <tbody id="proforma_invoiceItems">  
                              @php
                               $gsttype=$proforma_invoice->igst_type;
                              @endphp
                              @foreach($proforma_invoice_edits as $proforma_invoice_edit)
                           <tr>
                              @if($proforma_invoice_edit->Product!='')
                              <td>
                                 <select class="select2 form-control Product"  name="Product[]" style="width: 250px;">
                                    <option value="">Select Product</option>
                                    @foreach($products as $product){
                                     <option value="{{ $product->product_id }}"{{ (($proforma_invoice_edit->Product==$product->product_id)? 'selected':'') }}>{{ $product->product_name }}</option>
                                    @endforeach
                                 </select>
                                 <textarea class="form-control" style="padding-top:5px;" name="ProductDescription[]" placeholder="Product Description" style="width: 250px;">{{ $proforma_invoice_edit->ProductDescription }}</textarea>
                              </td>
                              <td><input type="number" class="form-control Cost" name="Cost[]" value="{{ $proforma_invoice_edit->Cost }}" style="width: 110px;">
                              </td>
                              <td><input type="number" class="form-control Quantity" name="Quantity[]" value="{{ $proforma_invoice_edit->Quantity }}" placeholder="Quantity" style="width: 110px;"></td>
                              <td>
                                 <input type="number" class="form-control GST" name="GST[]" placeholder="Total" value="{{ $proforma_invoice_edit->GST }}" style="width: 110px;">
                              </td>
                              <td class="IGSThide {{ (($proforma_invoice->igst_type!='1')?'hidden':'') }}"><input type="number" class="form-control IGST" name="IGST[]" value="{{ $proforma_invoice_edit->IGST }}" readonly style="width: 110px;"></td>
                              <td class="CGSThide {{ (($proforma_invoice->igst_type=='1')?'hidden':'') }}"><input type="number" class="form-control CGST" name="CGST[]" value="{{ $proforma_invoice_edit->CGST }}" readonly style="width: 110px;"></td>
                              <td class="SGSThide {{ (($proforma_invoice->igst_type=='1')?'hidden':'') }}"><input type="number" class="form-control SGST" name="SGST[]" value="{{ $proforma_invoice_edit->SGST }}" readonly style="width: 110px;"></td>
                              <td><input type="number" name="Amount[]" class="form-control Amount" readonly placeholder="Total" value="{{ $proforma_invoice_edit->Amount }}" style="width: 110px;"></td>
                              <td>
                                 <span type="button" class="btn btn-danger btn-sm RemoveProformaInvoiceItem text-white">
                                     <i class="fa fa-trash"></i> 
                                 </span>
                              </td>
                              @endif
                              @if($proforma_invoice_edit->Service!='')
                              <td>
                                 <select class="select2 form-control Product"  name="Service[]" style="width: 250px;">
                                    <option value="" disabled>Select Service</option>
                                    @foreach($services as $service){
                                     <option value="{{ $service->service_id }}"{{ (($proforma_invoice_edit->Service==$service->service_id)? 'selected':'') }}>{{ $service->service_name }}</option>
                                    @endforeach
                                 </select>
                                 <textarea class="form-control" style="padding-top:5px;" name="ServiceDescription[]" placeholder="Service Description" style="width: 250px;">{{ $proforma_invoice_edit->ServiceDescription }}</textarea>
                              </td>
                              <td><input type="number" class="form-control Cost" name="ServiceCost[]" value="{{ $proforma_invoice_edit->Cost }}" style="width: 110px;">
                              </td>
                              <td><input type="number" class="form-control Quantity" name="ServiceQuantity[]" value="{{ $proforma_invoice_edit->Quantity }}" placeholder="Quantity" style="width: 110px;"></td>
                              <td>
                                 <input type="number" class="form-control GST" name="ServiceGST[]" placeholder="Total" value="{{ $proforma_invoice_edit->GST }}" style="width: 110px;">
                              </td>
                              <td class="IGSThide {{ (($proforma_invoice->igst_type!='1')?'hidden':'') }}"><input type="number" class="form-control IGST" name="ServiceIGST[]" value="{{ $proforma_invoice_edit->IGST }}" readonly style="width: 110px;"></td>
                              <td class="CGSThide {{ (($proforma_invoice->igst_type=='1')?'hidden':'') }}"><input type="number" class="form-control CGST" name="ServiceCGST[]" value="{{ $proforma_invoice_edit->CGST }}" readonly style="width: 110px;"></td>
                              <td class="SGSThide {{ (($proforma_invoice->igst_type=='1')?'hidden':'') }}"><input type="number" class="form-control SGST" name="ServiceSGST[]" value="{{ $proforma_invoice_edit->SGST }}" readonly style="width: 110px;"></td>
                              <td><input type="number" name="ServiceAmount[]" class="form-control Amount" readonly placeholder="Total" value="{{ $proforma_invoice_edit->Amount }}" style="width: 110px;"></td>
                              <td>
                                 <span type="button" class="btn btn-danger btn-sm RemoveProformaInvoiceItem text-white">
                                     <i class="fa fa-trash"></i> 
                                 </span>
                        ...
                        <tfoot>
                           <tr>
                              ...
                              <td>Total Amount</td>
                              <td><input class="form-control TotalAmount" type="number" name="TotalAmount" readonly value="{{ $proforma_invoice->total_amount }}"  style="width: 110px;"></td>
                           </tr>
                           <tr class="IGSThide">
                              <td></td>
                              <td></td>
                              ...
                              <td class="SGSThide"></td>
                              <td>Total IGST</td>
                              <td><input class="TotalIGST form-control" type="number" name="TotalIGST" readonly value="{{ $proforma_invoice->total_igst }}"  style="width: 110px;"></td>
                              <td></td>
                           </tr>
                           <tr class="CGSThide">
                              <td></td>
                              ...
                              <td class="SGSThide"></td>
                              <td>Total CGST</td>
                              <td><input class="TotalCGST form-control" type="number" name="TotalCGST" readonly value="{{ $proforma_invoice->total_cgst }}"  style="width: 110px;"></td>
                              <td></td>
                           </tr>
                           <tr class="SGSThide">
                              <td></td>
                              ...
                              <td class="SGSThide"></td>
                              <td>Total SGST</td>
                              <td><input class="TotalSGST form-control" type="number" name="TotalSGST" readonly value="{{ $proforma_invoice->total_sgst }}"  style="width: 110px;"></td>
                              <td></td>
                           </tr>
                           <tr>
                              <td></td>
                              ...
                              <td class="SGSThide"></td>
                              <td>Grand Total</td>
                              <td><input class="form-control GrandTotal" type="number" name="GrandTotal" readonly value="{{ $proforma_invoice->grand_total }}"  style="width: 110px;"></td>
                              <td></td>
                              @endforeach
                           </tr>
                        </tfoot>
   ...
   <x-slot name="page_level_scripts">
      <script type="text/javascript">
         $(document).on('change','.Product',function(){
            ...
            $.post("{{ route('proforma_invoice_new_rate') }}",{"_token": "{{ csrf_token() }}","Product":Product},function(data){
               CurTr.find(".Cost").val(data);
               Cost=data;
               $.post("{{ route('proforma_invoice_new_gst') }}",{"_token": "{{ csrf_token() }}","Product":Product},function(data){
         ...
         $(document).on('change','.Service',function(){
            ...
            $.post("{{ route('proforma_invoice_new_rate_service') }}",{"_token":"{{ csrf_token() }}","Service":Service},function(data){
               CurTr.find(".Cost").val(data);
               Cost=data;
               $.post("{{ route('proforma_invoice_new_gst_service') }}",{"_token":"{{ csrf_token() }}","Service":Service},function(data){
                  CurTr.find(".GST").val(data);
                  GST=data;
                  $("tr .select2").select2();
                  var TotalProduct=(Cost*Quantity);
