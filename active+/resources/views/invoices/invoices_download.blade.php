...
<head>
   ...
   <body>
         ...
         <table style="width:100%">
            <tr>
               ...
               <th style="width:30%" align="right">
                  <p>Invoice #:  {{ $invoice_view->invoiceId }}</p>
                 
                  <p>Invoice Date : {{ $invoices->date_issue }}</p>
               </th>
            </tr>
         </table>
      </section>
      ...
      <section>
         <table style="width:100%">
            <tr>
               <th style="width:50%;" align="left">Invoice From :
               ...
               <th style="width:50%;" align="right">Invoice To :
               ...
               <tr>
                  @if($invoice_item->Product!='')
                     ...
                     @foreach($products as $product)
                        @if($invoice_item->Product==$product->product_id)
                           <td style="width:59%;">
                           {{ $product->product_name }}<span style="color:gray; font-style: italic; font-size: 13px;">{{ $invoice_item->ProductDescription }}</span><br>
                           </td>
                           <td style="width:12%;text-align: center;">{{ $invoice_item->Cost }}</td>
                           <td style="width:12%;text-align: center;">{{ $invoice_item->Quantity }}</td>
                           <td style="width:12%;text-align: center;">{{ $invoice_item->Amount }}</td>
                        @endif
                     @endforeach
                  @endif
                  @if($invoice_item->Service!='')
                     ...
                     @foreach($services as $service)
                        ...
                        <td style="width:59%;">
                        {{ $service->service_name }}<span style="color:gray; font-style: italic; font-size: 13px;">{{ $invoice_item->ServiceDescription }}</span><br>
                        </td>
                        <td style="width:12%;text-align: center;">{{ $invoice_item->Cost }}</td>
                        <td style="width:12%;text-align: center;">{{ $invoice_item->Quantity }}</td>
                        <td style="width:12%;text-align: center;">{{ $invoice_item->Amount }}</td>
      ...
      <section>
         <table style="width:100%;">
            <tr>        
               ...
               <span style="font-weight:normal;">{{ $invoices->total_amount }}.00</span></th>
            </tr>
            <br>
            <?php if($gsttype==2){
            echo '<tr>
               <th style="text-align: right;">CGST :
               <span style="font-weight:normal;">'.$invoices->total_cgst.'.00</span></th>
            ...
            echo '<tr>
               ...
               <span style="font-weight:normal;">'.$invoices->total_sgst.'.00</span></th>
            ...
            echo '<tr>
               ...
               <span style="font-weight:normal;">'.$invoices->total_igst.'.00</span></th>
            </tr>
            <br>';
            }
            ?>
            ...
            <tr>
               <th style="text-align: right;">Invoice Total :
               <span style="font-weight:normal;"> {{ $invoices->grand_total }}.00</span></th>
